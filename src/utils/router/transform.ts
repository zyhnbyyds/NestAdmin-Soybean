 ...
 * @returns trees
 */
export function transformRoutesToTreeOptions(routes: RouteManagement.RemoteRoute[]): TreeSelectOption[] {
  return routes.map(item => {
    const treeOption: TreeSelectOption = {
      label: item.meta.title,
      key: item.id,
      children: item.children ? transformRoutesToTreeOptions(item.children) : undefined
    };
    return treeOption;
  });
}
